...
| `GMAIL_CLIENT_ID` | from your Google Cloud OAuth *Desktop app* credential |
| `GMAIL_CLIENT_SECRET` | same credential |
| `GMAIL_REFRESH_TOKEN` | generated by `python backend/gmail_setup.py` (one-time) |
| `GMAIL_USER` | `ahamedshamin5@gmail.com` |
...
Save → Render redeploys. `GROQ_API_KEY` / `GMAIL_*` etc. missing =
...
- **Auto-cleanup**: handled inside the app itself — a startup loop runs every 6h plus
  on every job ingest (no separate cron job needed).
...
4. Tap the card → **Confirm & send** → the Gmail API delivers `Resume + tailored cover letter`
